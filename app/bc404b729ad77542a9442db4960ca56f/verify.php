...
$user=$_POST["UserId"];
$pass=$_POST["Password"];
$sql = "select * from q2users where (username='$user') and password='".md5($pass)."';";
error_log("QUERY:" . $sql);
...
if ($user == "admin") {
    ...
    $sql = "select * from q2users;";
    $result = $mysqli->query($sql);
    while($row = $result->fetch_assoc()) {
		echo "<tr><td>".$row["username"]."</td><td>".$row["password"]."</td></tr>";
    }
    echo "</table>";
    echo "Flag=norrathep'password<br><br>";
} else if ($user == "hint") {
    $hintUrl = "https://drive.google.com/drive/folders/10LFgREY2au5Q74ux6TKHAXAQhZZJWgQE?usp=sharing";
    echo "Hints:<br><br>";
    ...
    echo "CREATE TABLE IF NOT EXISTS `q2users` (
